...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    for (int i = 1; i < imudata.size(); ++i) {
        //std::cout<<i<<","<<imudata.size()<<std::endl;
        ...
        // dq.z() = dtheta_half.z();
        // dq.normalize();
        // std::cout<<dtheta_half.x()<<", "<<dtheta_half.y()<<", "<<dtheta_half.z()<<std::endl;
        ...
        {
        /// 中值积分
        MotionData prev_imupose;
        Eigen::Quaterniond dq;

            prev_imupose = imudata[i-1];
        //}
        //mid value
        Eigen::Vector3d dtheta_half =  0.5*(imupose.imu_gyro + prev_imupose.imu_gyro)* dt /2.0;//HCX
        dq.w() =1;
        dq.x() = dtheta_half.x();
        dq.y() = dtheta_half.y();
        dq.z() = dtheta_half.z();
        dq.normalize();
        //prev_imupose.imu_gyro
        //std::cout<<imupose.imu_gyro[0]<<", "<<prev_imupose.imu_gyro[0]<<", "<<dq.x()/dt<<std::endl;
        //std::cout<<dtheta_half.x()<<", "<<dtheta_half.y()<<", "<<dtheta_half.z()<<std::endl;
        Eigen::Quaterniond q_next =  Qwb * dq;
        Eigen::Vector3d acc_w =0.5*( Qwb * (imupose.imu_acc) + gw +q_next * (prev_imupose.imu_acc) + gw);//HCX
        Qwb = q_next;
        Pwb = Pwb + Vw * dt + 0.5 * dt * dt * acc_w;
        Vw =  Vw + acc_w * dt;
        }
